<table id="example2" class="table table-bordered table-hover">
	...
	<tr class="<?php echo cycle(array('bg1','bg'));?>">
		...
		?>
		<td width="100" align="center" class='stat_menu'>
		<a href="<?php echo $site_url.$active_controller;?>/set_submenu_status/<?php echo $row['id'].'/'.$id;?>" class="<?=strtolower($row[$field])?>"><?php echo $status_title; ?> </a>
		</td>
	   <?php elseif($field=='title'):?>
		<td class='view1'><a href="<?php echo $site_url.$active_controller;?>/view/<?php echo $row['id']?>" title='view details'><?php echo $row[$field]; ?> </a>
		</td>
	   ...
	   <?php elseif($field=='link'): ?>
		...
		<td class='actn-btn'>
		<a href="<?=$site_url.$active_controller;?>/edit/<?= $row['id'].'/'.$id; ?>" title="Click here for edit this item" class='edit'><span class="action_btn btn-primary"><i class="fa fa-pencil"></i></span></a>
		<a href="<?=$site_url.$active_controller;?>/del/<?= $row['id'].'/'.$id; ?>" title="Click here for delete this item" class='del'> <span class="action_btn btn-danger"><i class="fa fa-times"></i></span></a>
		</td>
		<?php elseif(is_array($grid_action) && !empty($grid_action)):?>	
		<td class='actn-btn' align="center">
		<?php foreach($grid_action as $gatype=>$gactn):?>
		<?php if($gatype === 'edit'):?> 
		<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'].'/'.$id; ?>" title="Click here for edit this item"><span class="action_btn btn-primary"><i class="fa fa-pencil"></i></span></a>
		<?php elseif($gatype === 'view'):?>
		<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'].'/'.$id; ?>" title="Click here for view details"><span class="action_btn btn-success"><i class="fa fa-search-plus"></i></span></a>
		...
		<a href="<?=$site_url.$active_controller.'/'.$gactn.'/'.$row['id'].'/'.$id; ?>" title="Click here for delete this item" class='del' ><span class="action_btn btn-danger"><i class="fa fa-times"></i></span></a>
		<?php else:?>
